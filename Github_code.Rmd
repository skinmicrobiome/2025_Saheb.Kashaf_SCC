...
```{r}
metadata <- read.csv("~/Desktop/Final_paper_vf/SuppTable1.csv", stringsAsFactors = FALSE)
...
ggsave(filename = "~/Desktop/Final_paper_vf/final_figures/suppfig1.png",
  plot = plot_age_scc,
  width = 8,
  height = 4)
```

Refine kraken output
```{r}

h5closeAll()

var.name=c("20230806_standard++_0.5_bracken")

# Import biom file from bracken generated via kraken-biom
merged_metagenomes <- import_biom(paste0("~/Desktop/Final_paper_vf/outputs/",var.name,".biom",sep=""))

# Process OTU table: clean sample names and retain samples present in metadata
otu.table <- merged_metagenomes@otu_table
colnames(otu.table) <- sub("\\..*", "", colnames(otu.table))
otu.table <- otu.table[, intersect(colnames(otu.table), metadata$Sample)]
otu.table <- otu_table(t(as.matrix(otu.table)), taxa_are_rows = FALSE)

# Subset metadata to match OTU table and create sample_data object
metadata_physeq <- metadata %>% 
  filter(Sample %in% rownames(otu.table)) %>%
  as.data.frame()
metadata_physeq$Patient_group=factor(metadata_physeq$Patient_group, levels=c("Low-Risk SCC","High-Risk SCC","SOTR"))
rownames(metadata_physeq) <- metadata_physeq$Sample
samples <- sample_data(metadata_physeq)

# Process taxonomy table: rename columns and add taxid
taxmat <- as.data.frame(merged_metagenomes@tax_table) %>%
  setNames(c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")) %>%
  mutate(taxid = rownames(.))
taxmat_matrix <- as.matrix(taxmat)

# Create phyloseq object
physeq_orig <- phyloseq(otu.table, tax_table(taxmat_matrix), samples)

# Remove human reads (and unclassified genera) and control samples
physeq_orig <- physeq_orig %>%
  subset_taxa(Genus != "g__Homo" & Genus != "g__") %>%
  subset_samples(site != "Ct")

# Save the phyloseq object
saveRDS(physeq_orig, file.path("~/Desktop/Final_paper_vf", paste0(var.name, ".rds")))
...
physeq_df_orig <- readRDS(paste0("~/Desktop/Final_paper_vf/", var.name, ".rds")) %>%
...
ps <- readRDS(file.path("~/Desktop/Final_paper_vf", paste0(var.name, ".rds")))
...
fig1<-plot_grid(final_plota,NULL,finalb,nrow=3, rel_heights = c(0.73,0.02, 0.25),labels = c("A","","B"), label_size = 20)
ggsave("~/Desktop/Final_paper_vf/final_figures/fig1.png",fig1, width=13, height=15)
```


Supp fig 2 - Full set of sites from fig 1b
...
ggsave("~/Desktop/Final_paper_vf/final_figures/suppfig2.png",suppfig2, width=12, height=12)
...
for (i in 1:2) {
  ...
  physeq <- readRDS(paste0("~/Desktop/Final_paper_vf/", var.name, ".rds"))
  ...
  variable_name <- paste("fig2a", i, sep = "_")
...
fig2a <- plot_grid(fig2a_1, fig2a_2, nrow = 2, rel_heights = c(0.4, 0.6))
print(fig2a)
...
# Create the plot
fig2c <- ggplot(average_abundance, aes(x = History.of.Warts.as.Adults, y = Abundance)) +
  theme_bw() +
...
legend2 <- plot_grid(get_legend(fig2c + guides(color = guide_legend(nrow = 1)) + theme(legend.position = "bottom")))
...
heatmapData = readRDS("~/Desktop/Final_paper_vf/HPV_tree/met_hpv_git.RDS")
...
tree_trim <- read.tree("~/Desktop/Final_paper_vf/HPV_tree/trim.tree")
...
fig2a2<-plot_grid(fig2a,legend2, nrow=2, labels=c("A",""), rel_heights = c(0.9,0.1), label_size = 20)
fig2_2<-plot_grid(fig2c,NULL,p2, NULL, ncol=4, labels=c("B","", "C",""), rel_widths = c(0.28, 0.05,0.7,0.05), label_size = 20)
fig2<-plot_grid(fig2a2, NULL, fig2_2,nrow=3, rel_heights=c(0.45,0.03, 0.52), label_size = 20)
fig2

ggsave("~/Desktop/Final_paper_vf/final_figures/fig2.png",fig2,width=17,height=16)
...
physeq_orig <- readRDS(paste0("~/Desktop/Final_paper_vf/", var.name, ".rds"))
...
suppfig3a <- ggplot(physeq_df, aes(x = subject_id, y = Abundance, fill = Genus)) +
  geom_bar(stat = "identity", position = "fill") +
...
suppfig3b <- ggplot(physeq_df, aes(x = Patient_group, y = Abundance, color = Patient_group)) +
...
plot_grid(
  suppfig3a+theme(legend.position="bottom"), NULL, suppfig3b+theme(legend.position="bottom"),
  labels = c("A", "B"),
  ncol = 3,               
  align = "hv",           
  rel_widths = c(0.57, 0.02, 0.41)
)
ggsave("~/Desktop/Final_paper_vf/final_figures/suppfig3.png",width = 13, height=10)
